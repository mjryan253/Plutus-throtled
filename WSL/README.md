...
Welcome to Plutus! This tool hunts for Bitcoin wallets that contain funds by generating keys on the GPU, deriving P2PKH addresses, and checking them against a bloom-filter database of known funded addresses. A GPU producer process feeds key pairs into a bounded queue; CPU consumer threads check addresses against the bloom and verify hits against the address list.
